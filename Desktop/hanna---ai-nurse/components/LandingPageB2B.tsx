            ...
            <nav className="fixed top-4 md:top-6 left-0 right-0 z-50 flex justify-center px-4 md:px-6 pointer-events-none">
                <div className="bg-white/90 backdrop-blur-3xl border border-white/60 rounded-full shadow-xl shadow-slate-900/5 px-6 py-3 md:px-8 flex items-center gap-12 max-w-4xl w-full justify-between transition-all duration-300 pointer-events-auto ring-1 ring-black/5">
                    ...
                    <div className="hidden md:flex items-center gap-8">
                        <button onClick={() => document.getElementById('solution')?.scrollIntoView({ behavior: 'smooth' })} className="text-text-secondary hover:text-hana-primary font-medium text-sm transition-colors font-sans">Platform</button>
                        ...
                        <button onClick={() => document.getElementById('timeline')?.scrollIntoView({ behavior: 'smooth' })} className="text-text-secondary hover:text-hana-primary font-medium text-sm transition-colors font-sans">Timeline</button>
                    </div>
